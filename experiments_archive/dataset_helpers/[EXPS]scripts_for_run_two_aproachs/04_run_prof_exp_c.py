...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Professor Experiment C: same scene budget + cap from baseline mean + margin."
    )
    ...
    cmd = [
        ...
        str(baseline_run),
        "--appear-cap-margin",
        str(float(args.appear_cap_margin)),
        "--chunk-size-mb",
    ...
    print_run_summary("PROF_C", run_dir)

    append_history(
        {
            "experiment": "prof_c_scene_budget_cap_margin",
            "run_dir": str(run_dir),
